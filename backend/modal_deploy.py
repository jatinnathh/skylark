"""
Minimal Modal deployment - only uploads the model file, not the entire backend directory
"""
import io
import base64
import modal
...
# Build image with dependencies and include the backend directory with the model
image = (
...
# No volume needed - model is baked into the image

@app.function(image=image)
@modal.asgi_app()
def serve():
    """Serve the FastAPI app with YOLO model"""
    from ultralytics import YOLO
    from PIL import Image, ImageDraw
    
    # Load model from the baked-in directory
    model = YOLO("/backend/runs/gcp_pose/weights/best.pt")
